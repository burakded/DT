...
export const useShareBrain = (brainId: string, userId: string) => {
  ...
  const embedLink =`<script> window.brainEmbed = "${baseUrl}/embed?brainId=${brainId}&userId=${userId}&brainName=${currentBrain?.name ?? "this"}&modeltype=gpt-3.5-turbo&mode=false&bubble=true&image=null&bubbleopen=false" </script> <script src="http://147.182.142.200:3000/bubble-embed.js"></script>`;
